...
install_packages_host gptfdisk glibc
...
if [[ $create_partitions =~ ^[Yy]$ ]]; then
    ...
    if ! [[ $efi_size =~ ^[0-9]+[GgMm]$ ]] || ! [[ $root_size =~ ^[0-9]+[GgMm]$ ]] || ! [[ $home_size =~ ^[0-9]+[GgMm]$ ]]; then
        print_color "31" "Invalid size format. Please use the format (e.g., 1G, 50G)."
        exit 1
    fi
